...
string:字符串；
"123"==123 true;
"0123"==0123 false;javascript认为�开头的证书常量当八进制数处理，所以后面的0123实际是八进制数；
...
8、Object.defineProperty()的属性描述符对象不必包含所�个特性。对于新创建的属性来说，默认的特性值是falseh或undefined。对于修改的已有属性来说，默认的特性值没有做任何修改。这个方法要么修改已有属性要么新建自有属性，但不能修改继承属性�
...
内联形式Javascript引用�
在XHTML�script></script>标签的内容被当做其他内容一样对待。如果Javascript代码包含��字符，那么这些字符就被解释成为XML标记。因此，如果要使用XHTML，最好把所有的Javascript代码放到一个CDATA部分里面�
<script><![CDATA[
    //这里是Javascript代码
]]></script>

外部文件中的脚本，使�script src="filepath"></script>
...
如果�毫秒的超时时间来调用setTimeout()那么指定的函数不会立刻执行，相反，会把它放到队列中，等到前面处于等待状态的事件处理程序充不执行完成后再调用它�
...
浏览器可能为表单字段和contenteditable元素支持自动拼写检查。添加spellcheck属性来显示开启拼写检查，而使用spellcheck=false来显示关闭该功能；Enter键另起一行，但不同的浏览器生成了不同的标记。有些开始了新的段落，而其他的只是插入了一�br/>元素�
